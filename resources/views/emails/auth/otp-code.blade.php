...
<body>
    <div class="email-shell">
        ...
        <div class="content">
            ...
            <p>Use this code to {{ $purposeLabel }} in <strong>{{ $appName }}</strong>.</p>

            <div class="code-block">{{ $otpCode }}</div>

            <div class="meta">
                <div class="meta-row"><strong>Code Sent At:</strong> {{ $sentAtPht }} {{ $timezoneLabel }}</div>
                <div class="meta-row"><strong>Expiration Time:</strong> {{ $expiresAtPht }} {{ $timezoneLabel }}</div>
            </div>
        </div>

        <div class="footer">
            <p style="margin: 0 0 8px;">Need help? Reply to this email or contact <a class="support" href="mailto:{{ $replyToAddress }}">{{ $replyToAddress }}</a>.</p>
            <p style="margin: 0;">This OTP is valid for {{ $validMinutes }} minutes based on the local server clock ({{ $timezoneLabel }}).</p>
        </div>
    </div>
</body>
</html>
